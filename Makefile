...
generate:  ## Generate python library from the OpenAPI specification
	mkdir -p out
	docker run --rm -v $(PWD):/local openapitools/openapi-generator-cli:latest generate -i /local/ShippingAPI_OAS.yaml -g python -o /local/out/pbclient --package-name pbclient
